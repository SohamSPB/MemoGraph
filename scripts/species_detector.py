...
def process_species(csv_path, trip_folder, log_path):
	...
	# Count of rows we've touched this run (whether mutated, skipped with bio
	# hint, or skipped without). Used only to space out periodic flushes.
	updated_count = 0

	def _flush():
		"""Incrementally flush every row in the labels CSV.

		The previous implementation built a parallel `updated_rows` list and
		flushed that — but since `updated_rows` only ever contained rows the
		for-loop had reached, an interrupt between flushes silently truncated
		the CSV. The fix is to flush the original `rows` list; this loop
		mutates dicts in `rows` in place, so all updates are already visible
		there and the un-iterated rows are preserved at their current values.
		"""
	...
	try:
		for row in rows:
			# Content duplicate: species_tags and species_boxes will be copied
			# from the canonical row by dedup_broadcast.py.
